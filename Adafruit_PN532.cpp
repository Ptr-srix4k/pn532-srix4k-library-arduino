...
/* Add some functions for basic SRIX4K tag support */
...
*/
/**************************************************************************/
bool Adafruit_PN532::srix4k_init(void) {
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 4))
  {
	Serial.println("Init command failed");   
	return false;
  }
  return true;
}
...
*/
/**************************************************************************/
bool Adafruit_PN532::srix4k_initiate_select(void) {
	
  uint8_t chip_id;
  
  //Send PN532 command InCommunicateThru
  //Send command INITIATE 0x06 0x00
  pn532_packetbuffer[0] = 0x42;
  pn532_packetbuffer[1] = 0x06;
  pn532_packetbuffer[2] = 0x00;
  if (! sendCommandCheckAck(pn532_packetbuffer, 3))
  {
	Serial.println("Initiate failed");  
    return false;
  }

  readdata(pn532_packetbuffer, 10);
  int offset = _usingSPI ? 6 : 7;  // Skip a response byte when using I2C to ignore extra data.
  if (pn532_packetbuffer[offset]!=0x00)
  {
	Serial.println("Initiate packet not zero"); 
    return false;
  }
  offset++;
  chip_id = pn532_packetbuffer[offset++];

  ///------------------------------------

  //Send PN532 command InCommunicateThru
  //Send command SELECT 0x0e chip_id
  pn532_packetbuffer[0] = 0x42;
  pn532_packetbuffer[1] = 0x0e;
  ...
  if (pn532_packetbuffer[offset]!=0x00)
  {
	Serial.println("Select packet not zero ");  
    return false;
  }
...
*/
/**************************************************************************/
bool Adafruit_PN532::srix4k_readblock(uint8_t address, uint8_t *block) {
	
  //Send PN532 command InCommunicateThru
  //Send command READ_BLOCK 0x08 Address
  pn532_packetbuffer[0] = 0x42;
  pn532_packetbuffer[1] = 0x08;
  pn532_packetbuffer[2] = address;
  if (! sendCommandCheckAck(pn532_packetbuffer, 3))
  {
	Serial.println("READ_BLOCK failed"); 
    return false;
  }
  ...
  if (pn532_packetbuffer[offset]!=0x00)
  {
	Serial.println("Error during READ_BLOCK command");   
    return false;
  }

  offset++;	
  block[0] = pn532_packetbuffer[offset]; offset++;	
  block[1] = pn532_packetbuffer[offset]; offset++;	
  block[2] = pn532_packetbuffer[offset]; offset++;	
  block[3] = pn532_packetbuffer[offset];
...
*/
/**************************************************************************/
bool Adafruit_PN532::srix4k_writeblock(uint8_t address, uint8_t block[4]) {
	
  //Send PN532 command InCommunicateThru
  //Send command WRITE_BLOCK 0x09 Address
  pn532_packetbuffer[0] = 0x42;
  pn532_packetbuffer[1] = 0x09;
  pn532_packetbuffer[2] = address;
  ...
  {
	Serial.println("WRITE_BLOCK failed");  
    return false;
  }
  
  //Redo Initate and Select, otherwise the next read fail (Why? Boh!)
  srix4k_initiate_select();
...
*/
/**************************************************************************/
bool Adafruit_PN532::srix4k_uid(uint8_t *block) {
	
  //Send PN532 command InCommunicateThru
  //Send command GET_UID 0x0b
  pn532_packetbuffer[0] = 0x42;
  pn532_packetbuffer[1] = 0x0b;
  if (! sendCommandCheckAck(pn532_packetbuffer, 2))
  {
	Serial.println("GET_UID failed");   
    return false;
  }
  ...
  if (pn532_packetbuffer[offset]!=0x00)
  {
	Serial.println("Error during GET_UID command");   
    return false;
  }
  ...
  block[1] = pn532_packetbuffer[offset]; offset++;	
  block[2] = pn532_packetbuffer[offset]; offset++;	
  block[3] = pn532_packetbuffer[offset]; offset++;	
  block[4] = pn532_packetbuffer[offset]; offset++;	
  block[5] = pn532_packetbuffer[offset]; offset++;	
  block[6] = pn532_packetbuffer[offset]; offset++;	
  block[7] = pn532_packetbuffer[offset]; offset++;
